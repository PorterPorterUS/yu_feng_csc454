...
let print_tree (t: parse_tree) () = 
  match t with
  | PT_nt (str, s) -> print_string str
  | PT_term str -> print_string str
  | PT_id str -> (print_string "id:")
  | PT_num str -> (print_string "num:")
  | _ -> ()
;;
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =
  ...
  | PT_nt ("TT", []) 
      -> lhs
  | PT_nt ("TT", [PT_nt ("ao", [PT_term ao]); term; tail])
      -> AST_binop (ao, lhs, (ast_ize_expr_tail (ast_ize_expr term) tail)) 
  | PT_nt ("FT", []) 
      -> lhs
  | PT_nt ("FT", [PT_nt ("mo", [PT_term mo]);fact; tail])
      -> AST_binop (mo, lhs, (ast_ize_expr_tail (ast_ize_expr fact) tail)) 
