...
public class SSSP {
	...
	private Surface build(RootPaneContainer pane, int an) {
		...
		if (an == PRINT_EVENTS) {
			s.setHooks(new Surface.EdgeRoutine() {
	...
	public static void main(String[] args) {
		...
		if (f != null) {
			...
			try {
				if (numThreads == 0) {
					s.DijkstraSolve();
					System.out.println("NOT MULTI_THRED");
					// s.DeltaSolve();
				} else {
					// s.DeltaSolve();
					System.out.println("START MULTI_THRED");
					s.multiThreadSolver(numThreads);
				}
	...
	public void run() {
		try {
			...
			if (dijkstra) {
				...
				s.DeltaSolve();
				System.out.println("START MULTI_THRED");
				s.multiThreadSolver(SSSP.numThreads);
			}
			c.unregister();
		} catch (Coordinator.KilledException e) {
		}
...
class Surface {
	...
	private class DistanceComparator implements Comparator<Vertex> {
		public int compare(Vertex v1, Vertex v2) {
			if (v1.distToSource < v2.distToSource)
				return -1;
			if (v2.distToSource < v1.distToSource)
				return 1;
			return 0;
		}
	}
				...
				for (int j = yl; j <= yh; ++j) {
					for (Vertex u : cb.get(i, j)) {
						if (v.hashCode() < u.hashCode()
							...
							int weight = (int) ((geom * (double) dist) + ((1.0 - geom) * (double) randWeight));
							// Pick u as neighbor.
	...
	/**************************************************************/
	// multi-threaded Delta-stepping solver.
	/**
	 * map: the concurrent hash table to store the DeltaWorker, it can be
	 * substituted by concurrentQueue.
	 */
	public static ConcurrentHashMap<DeltaWorker, Integer> map;
	/**
	 * Queues: in my design, I used same amount of queues as the number of buckets,
	 * in this way, different threads won't infer each other's queue and it should
	 * be quicker than only have a common queue.
	 */
	public static ArrayList<ConcurrentLinkedQueue<T_Request>> queues;
	/**
	 * tBuckets: Same as the buckets, but I used a concurrentHashMap, so it can
	 * eliminate the multi-threads interruption issue.
	 */
	public static ArrayList<ConcurrentHashMap<Vertex, Boolean>> tBuckets;
	/**
	 * vertexIndexMap: this is used to index each vertex with the right thread, so,
	 * we will put vertex into the right corresponding queue.
	 * 
	 */
	public static ConcurrentHashMap<Vertex, Integer> vertexIndexMap;
	/**
	 * this lock is only used for creating threads.
	 */
	public static Object lock = new Object();
	/**
	 * threads: the block to store the threads object and wait to "join" different
	 * threads, Only for debug purpose.
	 */
	public static ArrayList<Thread> threads = new ArrayList<>();
	/**
	 * number of buckets. (Multi-threaded version)
	 */
	int numThreadBuckets;
	/**
	 * the delta value for "delta stepping" algorithm. (Multi-threaded version)
	 */
	int deltaThread;

	public void multiThreadSolver(int nThread) {
		numThreadBuckets = 2 * degree;
		deltaThread = maxCoord / degree;
		map = new ConcurrentHashMap<>();
		// each thread has its only queue;
		queues = new ArrayList<>(nThread);
		// map vertex with bucket;
		vertexIndexMap = new ConcurrentHashMap<>(vertices.length);
		// create the ConcurrentHashMap to store the Vertexes.
		tBuckets = new ArrayList<>(numThreadBuckets);
		// for each delta stepping, create a bucket.
		for (int i = 0; i < numThreadBuckets; ++i) {
			tBuckets.add(new ConcurrentHashMap<>());
		}
		// put source vertex into bucket No.1
		tBuckets.get(0).put(vertices[0], true);
		// for each thread, create a queue for each thread.
		for (int i = 0; i < nThread; i++) {
			queues.add(new ConcurrentLinkedQueue<>());
		}
		// divide up how many vertexs do each thread need to be handled.
		int vertexPerThread = vertices.length / nThread;
		if (vertices.length % nThread != 0) {
			vertexPerThread += 1;
		}
		// created barrier for all threads.
		CyclicBarrier b = new CyclicBarrier(nThread);
		// create all threads.
		for (int i = 0; i < nThread; i++) {
			synchronized (lock) {
				map.put(new DeltaWorker(b, i * vertexPerThread, (i + 1) * vertexPerThread, i, numThreadBuckets,
						deltaThread), 0);
			}
		}
		// let each threads to run.
		...
		// let each thread to join.
		// this part can actually no need.
	...
	private class DeltaWorker implements Runnable {
		/**
		 * set: is used for store the vertex this thread need to be handled. no need to
		 * make it concurrent because this single thread will work on it.
		 */
		private LinkedHashSet<Vertex> set;
		/**
		 * barrier: the object reference to stop this thread proceed at the "right"
		 * time.
		 */
		private CyclicBarrier barrier;
		/**
		 * from: the beginning index of vertices this thread needs to be handled.
		 */
		private int from;
		/**
		 * to: the end index of vertices this thread needs to be handled.(exclusive)
		 */
		private int to;
		/**
		 * index: the queue index of this thread corresponding to.
		 */
		private int index;
		/**
		 * the copy of numThreadBuckets;
		 */
		private int nBuckets;
		/**
		 * the copy of deltaThread;
		 */
		private int tDelta;

		/**
		 * the Construction of worker thread.
		 */
		public DeltaWorker(CyclicBarrier b, int f, int t, int ind, int n, int d) {
			from = f;
			to = t;
			barrier = b;
			nBuckets = n;
			...
			for (int i = from; i < to && i < vertices.length; i++) {
				set.add(vertices[i]);
				vertexIndexMap.put(vertices[i], index);
			}
		}

		/**
		 * override the run funciton.
		 */
		@Override
		public void run() {
			int i = 0;
			try {
				// this thread will continue running until its stop condition.
				while (true) {
					// the vertex that we removed from this current bucket, it used for
					// "heavy edge relaxing".
					// (this can be optimized using a simple arraylist.)
					ConcurrentHashMap<Vertex, Boolean> removed = new ConcurrentHashMap<>();
					while (tBuckets.get(i).size() > 0) {
						// every time, it will remove the previous queue.
						queues.get(index).clear();
						barrier.await();
						// find the light edges to relax, and added the vertex to the remove
						// array list.
						tFindRequests(tBuckets.get(i), true, removed); // light relaxations
						// Move all vertices from bucket i to removed list.
						barrier.await();
						// Relax all the requsts.
						for (T_Request req : queues.get(index)) {
							req.relax();
						}
						// wait all the request finished.
						barrier.await();
					}
					// again, to make sure all the thread finished their own schedules.
					barrier.await();
					queues.get(index).clear();
					barrier.await();
					// find the "heavy" edges that we need to relax.
					tFindRequests(removed, false, null);
					barrier.await();
					// relaxed the heavy edges.
					for (T_Request req : queues.get(index)) {
						req.relax();
					}
					barrier.await();
					...
					do {
						j = (j + 1) % nBuckets;
					} while (j != i && tBuckets.get(j).size() == 0);
					if (i == j) {
						// Cycled all the way around; we're done
						break; // for (;;) loop
					}
		...
		// multi-threaded version of FindRequest.
		private void tFindRequests(ConcurrentHashMap<Vertex, Boolean> bucket, boolean light,
				ConcurrentHashMap<Vertex, Boolean> removed) {
			for (Vertex v : bucket.keySet()) {
				if (set.contains(v) && bucket.get(v)) {
					...
					for (Edge e : v.neighbors) {
						// System.out.println("Edge: " + e.weight + "; tDelta: " + tDelta);
						if ((light && e.weight <= tDelta) || (!light && e.weight > tDelta)) {
							Vertex o = e.other(v);
							// System.out.println("add: " + o.toString());
							queues.get(index).add(new T_Request(o, e));
						}
					}
				}
			}
	...
	// the multi-thread version of request.
	class T_Request {
		private Vertex v;
		private Edge e;
		...
		public void relax() throws Coordinator.KilledException {
			...
			if (altDist < v.distToSource) {
				// Yup; better path home.
				tBuckets.get((int) ((v.distToSource / deltaThread) % numThreadBuckets)).remove(v);
				v.distToSource = altDist;
				if (v.predecessor != null) {
					v.predecessor.unselect();
				}
				v.predecessor = e;
				e.select();
				tBuckets.get((int) ((altDist / deltaThread) % numThreadBuckets)).put(v, true);
			}
		}

		// constructor.
		public T_Request(Vertex V, Edge E) {
			v = V;
			e = E;
		}
	}

	/********** End of the multi-threaded program *****************/
	/**************************************************************/
	/**************************************************************/
...
class Animation extends JPanel {
	...
	private int yPosition(int y) {
		return (int) (((double) s.maxCoord - (double) y) * (double) height / ((double) s.maxCoord)) + border;
	}
	...
	public void paintComponent(final Graphics g) {
		...
		s.forSource(new Surface.VertexRoutine() {
			public void run(int x, int y) {
				...
				g.fillOval(xPosition(x) - dotsize, yPosition(y) - dotsize, dotsize * 2, dotsize * 2);
				g2.setPaint(Color.black);
				g2.setStroke(new BasicStroke(2));
				g.drawOval(xPosition(x) - dotsize, yPosition(y) - dotsize, dotsize * 2, dotsize * 2);
			}
		});
	}
...
class UI extends JPanel {
	...
	// Constructor
	//
	public UI(Coordinator C, Surface S, Animation A, long SD, int NT, RootPaneContainer pane) {
		final UI ui = this;
		coordinator = C;
		surface = S;
		animation = A;
